...
const WizardUserFormSecondPage = (props) => (
  <>
    <Typography className={"mt-3"} variant={"h6"}>
      Information de contact (affichées pour les
      {localStorage.getItem("status") === "volunteer" ? "associations" : "participants"})
    </Typography>
    <div className="form-group">
      <Field
        component={TextField}
        {...props}
        value={props.formik.values.contactEmail ? props.formik.values.contactEmail : ''}
        variant={"filled"}
    ...
    <div className="form-group">
      <Field
        component={TextField}
        value={props.formik.values.contactPhone ? props.formik.values.contactPhone : ''}
        {...props}
        variant={"filled"}
